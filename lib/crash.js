...
function failfast (val, predicate /* , ...message */) {
  ...
  crash(slice.call(arguments, 2)) // slow on V8, but we're crashing anyway!
}
...
function crash (/* ...message */) {
  var message = [ 'exit on' ]
  var args
  if (!arguments.length) message.push('undocumented error')
  else {
    args = Array.isArray(arguments[0]) ? arguments[0] : slice.apply(arguments)
    message = message.concat(args) // slow on V8, but...
  }
